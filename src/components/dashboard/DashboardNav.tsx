...
function PlanBadge({ tier }: { tier: Plan }) {
  ...
  return (
    <span className="text-[10px] font-medium px-1.5 py-px rounded-full bg-slate-800/50 border border-slate-700/40 text-slate-400 uppercase tracking-wide">
      Free
    </span>
  );
}
...
function MobileNavXP() {
  ...
  return (
    <div data-tour="xp-level" className="sm:hidden flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-violet-950/40 border border-violet-800/25 flex-shrink-0">
      ...
      <span className="text-slate-400 text-[10px] select-none leading-none">·</span>
    ...
    <Link
      ...
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
        pathname === href
          ...
          : "text-slate-400 hover:text-slate-300 hover:bg-violet-950/30"
      }`}
    >
      {icon}
          ...
          <div className="hidden lg:flex items-center gap-3 flex-shrink-0">
            <div className="flex items-center gap-0.5">
              <button
                onClick={() => router.back()}
                aria-label="Go back"
                className="w-7 h-7 flex items-center justify-center rounded-full text-slate-400 hover:text-slate-300 hover:bg-violet-950/40 transition-all"
              >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="relative flex-shrink-0" ref={menuRef}>
              ...
              >
                ...
                <ChevronDown
                  className={`hidden sm:block w-3 h-3 text-slate-400 transition-transform duration-200 ${menuOpen ? "rotate-180" : ""}`}
                />
              </button>
...
function MenuItem({ icon, label, href, external, onClick, badge, muted }: MenuItemProps) {
  ...
  const inner = (
    <>
      <span className={muted ? "text-slate-400" : "text-slate-400 group-hover:text-violet-400 transition-colors"}>
        {icon}
      </span>
